...
function Admin(props) {
      ...
      const handleDelete = async (id) => {
        ...
        const filteredItems = prods.filter(i => i.id !== id);
        setProds(filteredItems);
      }
    ...
    return ( 
        <>
            ...
            <Link className="btn btn-success fw-bold d-block mx-auto mb-4" style={{width: '150px'}} to='/add'>Add Product</Link>
                    ...
                    {prods.map(product => {
                      return (
                        <tr key={product.id}>
                          ...
                          <td>{product.price}</td>
                          <td>
                            <Link className="btn btn-primary mx-2" to={`/editProduct/${product.id}`}>
                            ...
                            <button className="btn btn-danger mx-2" onClick={()=>handleDelete(product.id)}>
